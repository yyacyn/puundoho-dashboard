...
export default function BendaharaOverview() {
    ...
    // Calculate totals from data instead of relying on summary fields (matching ApbDes.jsx logic)
    const totalPendapatan = useMemo(() => pendapatanData.reduce((sum, d) => sum + d.jumlah, 0), [pendapatanData])
    const totalPengeluaran = useMemo(() => pengeluaranData.reduce((sum, d) => sum + d.jumlah, 0), [pengeluaranData])
    ...
    const pendChange = calcChange(totalPendapatan, prevYearData?.total_pendapatan)
    const pengChange = calcChange(totalPengeluaran, prevYearData?.total_pengeluaran)

    const metricCards = [
        {
            label: 'Total Pendapatan',
            value: formatRupiahShort(totalPendapatan),
            change: pendChange.text,
            up: pendChange.up,
        },
        {
            label: 'Total Pengeluaran',
            value: formatRupiahShort(totalPengeluaran),
                ...
                {metricCards.map((card) => (
                    <div key={card.label} className="flex flex-col gap-4 p-5 rounded-xl bg-[#141417] border border-[#1F1F23]">
                        ...
                        <div className="flex flex-col gap-1">
                            <span className="text-white leading-tight" style={{ fontFamily: 'DM Mono, monospace', fontSize: 18, fontWeight: 600 }}>
                                {card.value}
                            </span>
