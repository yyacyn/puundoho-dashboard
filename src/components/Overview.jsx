...
const metricCards = [
    { label: 'Total Penduduk', value: '2,847', change: '+3.2%', up: true, live: true },
    { label: 'Kasus Stunting', value: '38', change: '-5.1%', up: false, live: false },
    { label: 'Berita Tayang', value: '124', change: '+8.5%', up: true, live: false },
    { label: 'Pengaduan Aktif', value: '9', change: '-2.0%', up: false, live: true },
]
...
const transactions = [
    { initials: 'BN', name: 'Budi Nugroho', desc: 'Pendaftaran penduduk baru', badge: 'Selesai', badgeColor: 'bg-green-500/10 text-green-400', amount: '+1 Penduduk', amountColor: 'text-green-400' },
    { initials: 'SR', name: 'Siti Rahayu', desc: 'Laporan kasus stunting RT 03', badge: 'Proses', badgeColor: 'bg-orange-500/10 text-orange-400', amount: '1 Kasus', amountColor: 'text-[#ADADB0]' },
    { initials: 'AW', name: 'Ahmad Wahyudi', desc: 'Pengaduan jalan rusak RW 02', badge: 'Baru', badgeColor: 'bg-blue-500/10 text-blue-400', amount: '1 Pengaduan', amountColor: 'text-[#ADADB0]' },
    { initials: 'DK', name: 'Dinas Keuangan', desc: 'Input realisasi APBDes Q1 2026', badge: 'Selesai', badgeColor: 'bg-green-500/10 text-green-400', amount: 'Rp 48,5 Jt', amountColor: 'text-green-400' },
]
...
const galleryItems = [
    { iconColor: 'text-[#4A4A4E]', gradientFrom: '#1A1A1D', gradientTo: '#2A2A2E', title: 'Gotong Royong Desa', desc: 'Kegiatan bersih desa setiap bulan bersama warga' },
    { iconColor: 'text-[#298064]', gradientFrom: '#1F1F23', gradientTo: '#29806433', title: 'Festival Budaya 2025', desc: 'Perayaan hari jadi desa dengan pertunjukan seni' },
    { iconColor: 'text-green-400', gradientFrom: '#22C55E18', gradientTo: '#1F1F23', title: 'Panen Raya Bersama', desc: 'Dokumentasi panen raya warga kelompok tani' },
]
        ...
        <div className="flex flex-col gap-7 px-10 py-8">
                ...
                {metricCards.map((card) => (
                    <div key={card.label} className="flex flex-col gap-4 p-5 rounded-xl bg-[#141417] border border-[#1F1F23]">
                        <div className="flex items-center justify-between">
                            <span className="text-[#6B6B70] text-xs font-medium tracking-wide uppercase">{card.label}</span>
                            {card.live && (
                                <div className="flex items-center gap-1.5">
                                    <span className="w-1.5 h-1.5 rounded-full bg-green-400" style={{ boxShadow: '0 0 8px #22C55E88' }} />
                        ...
                        <div className="flex items-center gap-1">
                            ...
                            <span className={`text-xs font-medium ${card.up ? 'text-green-400' : 'text-red-400'}`}>{card.change}</span>
                        </div>
                    </div>
                ))}
            </div>
            ...
            <div className="flex items-center gap-3">
                {[
                    ...
                    <button key={label} className="flex items-center gap-2 px-3.5 py-2 rounded-md bg-[#1A1A1D] text-[#ADADB0] text-xs font-medium hover:bg-[#2A2A2E] transition-colors">
                    ...
                    {barData.map((bar) => (
                        <div key={bar.label} className="flex flex-col items-center gap-2 flex-1 h-full justify-end">
                            <div
                                ...
                                style={{ height: bar.height, background: 'linear-gradient(180deg, #298064 0%, #3aad85 100%)' }}
                            />
            ...
            <div className="flex flex-col gap-4">
                <div className="flex items-center justify-between">
                    ...
                    <button className="text-[#298064] text-xs font-medium flex items-center gap-1 hover:opacity-80">
                    ...
                    {transactions.map((item) => (
                        <div key={item.name} className="flex items-center justify-between p-4 rounded-[10px] bg-[#141417] border border-[#1F1F23]">
                            ...
                            <div className="flex items-center gap-4">
                                ...
                                <span className={`text-sm font-medium text-right ${item.amountColor}`} style={{ fontFamily: 'DM Mono, monospace', minWidth: 100 }}>
                                    {item.amount}
                                </span>
                            </div>
                        </div>
                ...
                <div className="flex items-center justify-center gap-2 pt-1">
                    {[1, 2, 3].map((page) => (
                        <button key={page} className={`w-8 h-8 rounded-md flex items-center justify-center text-xs font-medium transition-colors ${page === 1 ? 'bg-[#298064] text-white' : 'bg-[#1A1A1D] text-[#6B6B70] hover:bg-[#2A2A2E]'}`}>
                            {page}
                        </button>
                    ))}
            ...
            <div className="flex flex-col gap-4">
                ...
                <div className="rounded-xl overflow-hidden border border-[#1F1F23] bg-[#111113]">
                    ...
                    {tableRows.map((row, i) => (
                        <div key={row.name} className={`grid grid-cols-5 items-center px-5 py-3.5 ${i < tableRows.length - 1 ? 'border-b border-[#1F1F23]' : ''} bg-[#141417] hover:bg-[#1A1A1D] transition-colors`}>
                            <div className="flex items-center gap-3">
                            ...
                            <span className="text-[#ADADB0] text-[13px]" style={{ fontFamily: 'DM Mono, monospace' }}>{row.count}</span>
                            <span className={`text-xs font-medium ${parseInt(row.stunting) >= 5 ? 'text-red-400' : 'text-[#ADADB0]'}`} style={{ fontFamily: 'DM Mono, monospace' }}>
                                {row.stunting} kasus
                            </span>
                            <span className={`inline-flex w-fit px-2.5 py-1 rounded-full text-xs font-medium ${row.statusColor}`}>{row.status}</span>
            ...
            <div className="flex flex-col gap-4 pb-8">
                ...
                <div className="grid grid-cols-3 gap-4">
                    {galleryItems.map((item) => (
                        <div key={item.title} className="rounded-xl overflow-hidden border border-[#1F1F23] bg-[#111113] flex flex-col">
                            <div className="h-[140px] flex items-center justify-center" style={{ background: `linear-gradient(135deg, ${item.gradientFrom} 0%, ${item.gradientTo} 100%)` }}>
